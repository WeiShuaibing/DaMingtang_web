<template>
    <div>
      ...
      <div style="margin-top: 50px;">
        ...
        <el-button type="success" @click="submitData()" style="width: 150px;" :loading="submitLoading">更  新</el-button>
      </div>
    </div>
</template>
...
export default {
  ...
  created: function () {
    this.getMaterialInfo(this.editPage)
  ...
  methods: {
    getMaterialInfo (id) {
      ...
      // 根据id查询物料信息
      this.axios.get(this.GLOBAL.BASE_URL + '/material/selectMaterialById?id=' + id)
        .then(res => {
          if (res.data.code === 20000) {
            this.returnData = res.data.data
            this.ma_num = this.returnData.ma_num
            this.ma_name = this.returnData.ma_name
            this.ma_grade = this.returnData.ma_grade
            this.ma_long = this.returnData.ma_long
            this.ma_wide = this.returnData.ma_wide
            this.ma_high = this.returnData.ma_high
            this.ma_area = this.returnData.ma_area
            this.ma_volume = this.returnData.ma_volume
            this.ma_price = this.returnData.ma_price
            this.ma_invented = this.returnData.ma_invented
            this.ma_bancai_type = this.returnData.ma_bancai_type
            this.ma_unit = this.returnData.ma_unit
            this.ma_attribute = this.returnData.ma_attribute
            this.ma_wuliao_type = this.returnData.ma_wuliao_type // ******
            this.ma_weight = this.returnData.ma_weight
            this.ma_remark = this.returnData.ma_remark
          } else {
            this.$message({
              message: '警告哦，数据查询出现问题 ！',
              type: 'warning'
            })
          }
        })
    ...
    sendData () {
      ...
      this.axios.post(this.GLOBAL.BASE_URL + '/material/updateMaterial', {
        id: this.id,
        ma_num: this.ma_num,
